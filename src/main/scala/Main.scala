...
object Main extends App {
  ...
  val reader = input.uri.asCsvReader[MovieRow](rfc.withHeader("budget", "genres", "id", "popularity", "production_companies", "release_date", "revenue"))
...
object IngestModel {
  ...
  implicit val movieRowDecoder: RowDecoder[MovieRow] = RowDecoder.ordered { (budget: Long, genres: String, id: String, popularity: Double, productionCompanies: String, release: LocalDate, revenue: Long) =>
  ...
  case class MovieRow(
    ...
    lazy val  genreIds: Set[String] = Try(Json.parse(genreIdsJson).as[List[JsObject]].map((_ \ "id").as[String]).toSet).getOrElse(Set.empty)
    lazy val  productionCompanyIds: Set[String] = Try(Json.parse(genreIdsJson).as[List[JsObject]].map((_ \ "id").as[String]).toSet).getOrElse(Set.empty)
...
object OutputModels {
    ...
    def sum(a: ProductionCompanyDetails): ProductionCompanyDetails = {
      a.copy(
        ...
        metadata = metadata.copy(
          movieIds = metadata.movieIds ++ a.metadata.movieIds, genreIds = metadata.genreIds ++ a.metadata.genreIds
        )
      )
    }
  }
  ...
  case class GenreDetails(
    ...
    def sum(a: GenreDetails): GenreDetails = {
      a.copy(
        ...
        revenue = a.revenue + this.revenue,
        avgPopulatarity = (a.avgPopulatarity + this.avgPopulatarity) / 2,
        metadata = metadata.copy(movieIds = this.metadata.movieIds ++ a.metadata.movieIds, productionCompanyIds = metadata.productionCompanyIds ++ a.metadata.productionCompanyIds)
      )
    }
  }
  ...
  private def newProductDetailsMap: ProductCompanyDetailsMap = SortedMap.empty[Int, Array[Map[String, ProductionCompanyDetails]]]

  // Year -> Map(ProductionCompanyId -> List of Details)
  type GenreDetailsMap = BucketedYearMap[GenreDetails]
  private def newGenreDetailsMap: GenreDetailsMap = SortedMap.empty[Int, Array[Map[String, GenreDetails]]]
  ...
  case class DetailCollector(moviesSeen: Int = 0, genresDetails: GenreDetailsMap = newGenreDetailsMap, prodCompanyDetails: ProductCompanyDetailsMap = newProductDetailsMap) {
    def accept(row: IngestModel.MovieRow): DetailCollector = {
      ...
      this.copy(moviesSeen = this.moviesSeen + 1, genresDetails = updatedGenreDetails, prodCompanyDetails = updatedProdCompanyDetails)
    }
